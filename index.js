...
conn.on('qr', qr =>
{
   ...
   console.log(`[ ${moment().format("HH:mm:ss")} ] Nanas Ready scan now!`);
});
...
conn.on('user-presence-update', json => console.log(`[ ${moment().format("HH:mm:ss")} ] => bot by @nasrullah_ns00`))
conn.on('message-status-update', json =>
{
   const participant = json.participant ? ' (' + json.participant + ')' : '' // participant exists when the message is from a group
   console.log(`[ ${moment().format("HH:mm:ss")} ] => bot by @nasrullah_ns10`)
})
...
conn.sendMessage(id, 'lu yg bacot_-#' ,MessageType.text);
}
else if (text == 'Tst'){
...
conn.sendMessage(id, 'hmm... ' ,MessageType.text);
}
else if (text == 'Mas'){
conn.sendMessage(id, 'Ya?' ,MessageType.text);
}
